...
ui <- fluidPage(
  # App title
  titlePanel("Automatic Australian Livestock forecasts"),
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      sliderInput("trainRange",
                  ...
                  step = 1,
                  sep = ""),
      selectInput(inputId = "model_id", label = "Model", choices = c("Mean", "Naive", "AR")),
...
server <- function(input, output) {
  output$timeseries_plot <- renderPlot({
    ...
    models <- list(Mean = MEAN, Naive = NAIVE, AR = AR)
